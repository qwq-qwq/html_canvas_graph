...
function updateRippleEffect() {
  ...
  if (now - ripple.lastWaveTime > ripple.waveInterval) {
    ...
    ripple.waves.push({
      ...
      frequency: 0.5, //3 + Math.random() * 6,   // Более низкая частота для плавности
      ...
      rotationSpeed: 0.0001,//(Math.random() - 0.5) * 0.01, // Скорость вращения волны
  ...
  if (ripple.waves.length === 0 || Math.random() < 0.0005) {
    ...
    // Периодически очищаем холст для предотвращения перегрузки
